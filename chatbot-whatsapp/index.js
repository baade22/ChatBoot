// index.js (Versão Final e Corrigida para Painel e Socket.IO)
...
// Ação Corretiva: Servir arquivos estáticos da pasta atual
...
//======================================================================
// LÓGICA DO CHATBOT (HANDLERS E FUNÇÕES AUXILIARES)
//======================================================================
...
async function handleRegistration(from, message, db) {
    ...
    if (userInput?.toLowerCase() === 'pular' && !currentStep.required) {
        ...
        if (currentStep.required && !userInput) {
            await sendWhatsAppMessage(from, { type: 'text', text: { body: `Esta informação é necessária. ${currentStep.question}` } });
            return;
        }
    ...
    if (nextStepIndex < registrationSteps.length) {
        ...
        try {
            await sendWhatsAppMessage(from, { type: 'text', text: { body: "Seu cadastro foi realizado com sucesso! ✅" } });

            if (userState.resumeAfterRegistration) {
                await sendWhatsAppMessage(from, { type: 'text', text: { body: "Agora vamos continuar de onde parámos..." } });
...
async function handleMemberVerification(from, message, db) {
    ...
    switch (userState.step) {
        case 'ask_name':
            await sendWhatsAppMessage(from, { type: 'text', text: { body: `Ok, um momento... 🔍` } });
            try {
    ...
    switch (userState.step) {
        ...
        case 'confirm_identity':
            if (receivedId === 'contrib_confirm_yes') {
                ...
                await sendWhatsAppMessage(from, { type: 'text', text: { body: "Ok. Para tentar novamente, por favor, escolha a opção de contribuição no menu." } });
                await db.collection('userSessions').deleteOne({ _id: from });
            }
            break;
        case 'ask_value':
            ...
            if (isNaN(valor) || valor < 1) {
                await sendWhatsAppMessage(from, { type: 'text', text: { body: "Valor inválido. Por favor, digite um valor numérico (Ex: 50,00)." } });
                return;
            }
            ...
            try {
                ...
                const responseMsg = paymentType === 'boleto' ? `Boleto gerado! ✅\n\n*Link:* ${resultData.link}\n\n*Linha Digitável:*\n\`\`\`${resultData.linhaDigitavel}\`\`\`` : `PIX gerado! ✅\n\nCopie o código abaixo e cole no seu app do banco:\n\n\`\`\`${resultData.emv}\`\`\``;
    ...
    switch (userState.step) {
        case 'select_event':
            ...
            if (!eventoSelecionado) {
                await sendWhatsAppMessage(from, { type: 'text', text: { body: "Opção de evento inválida. Por favor, selecione uma da lista." } });
                const eventRows = eventosFromDB.map(e => ({ id: e._id.toString(), title: e.title, description: e.description }));
                const eventListMessage = { type: "interactive", interactive: { type: "list", header: { type: "text", text: "Eventos Disponíveis" }, body: { text: "Temos estes eventos abertos. Selecione um:" }, action: { button: "Ver Eventos", sections: [{ title: "Nossos Eventos", rows: eventRows }] } } };
            ...
            break;
        case 'ask_name':
            await sendWhatsAppMessage(from, { type: 'text', text: { body: `Ok, a verificar seu cadastro por "${receivedText}"... 🔍` } });
            try {
                const response = await axios.get(EKLESIA_API_URL, { params: { codIgreja: 1, nome: receivedText }, headers: { 'Authorization': `Bearer ${EKLESIA_API_TOKEN}` } });
                if (response.data?.registros?.length > 0) {
                    const membroData = response.data.registros[0];
        ...
        case 'confirm_identity':
            ...
            if (receivedId === 'event_confirm_yes') {
                if (evento.paid) {
                    ...
                    await sendWhatsAppMessage(from, { type: 'text', text: { body: `Perfeito! O valor da inscrição para "${evento.title}" é de ${valorFormatado}. Como deseja pagar?` } });
                    const paymentOptions = { type: "interactive", interactive: { type: "button", body: { text: `Forma de Pagamento` }, action: { buttons: [{ type: "reply", reply: { id: "dizimo_boleto", title: "Boleto Bancário" } }, { type: "reply", reply: { id: "dizimo_pix", title: "PIX" } }] } } };
                    await sendWhatsAppMessage(from, paymentOptions);
                } else {
                    await sendWhatsAppMessage(from, { type: 'text', text: { body: `Inscrição para o evento gratuito "${evento.title}" realizada com sucesso! ✅` } });
                    await db.collection('eventRegistrations').insertOne({ eventoId: evento._id.toString(), eventoNome: evento.title, nomeMembro: userState.membroData.nome, dataPagamento: new Date(), formaPagamento: 'Gratuito', valor: 0, status: 'CONCLUÍDO' });
                    await enviarRelatorioInscricoes(db);
                    await db.collection('userSessions').deleteOne({ _id: from });
                }
            } else {
                await sendWhatsAppMessage(from, { type: 'text', text: { body: "Ok. Para tentar com outro nome, escolha a opção de eventos no menu." } });
                await db.collection('userSessions').deleteOne({ _id: from });
            }
            break;
        case 'ask_payment':
            ...
            else { await sendWhatsAppMessage(from, { type: 'text', text: { body: "Opção de pagamento inválida." } }); return; }
            await sendWhatsAppMessage(from, { type: 'text', text: { body: `A gerar sua forma de pagamento via ${paymentType}, aguarde... 🏦` } });
            try {
                ...
                await sendWhatsAppMessage(from, { type: 'text', text: { body: "Não foi possível gerar sua forma de pagamento." } });
            } finally {
                await db.collection('userSessions').deleteOne({ _id: from });
            }
            break;
...
app.post('/webhook', (req, res) => {
    // Ação 1: Responder imediatamente ao WhatsApp
    res.sendStatus(200);

    // Ação 2: Processar a mensagem em segundo plano
    const process = async () => {
        ...
        try {
                ...
                if (receivedId) {
                    ...
                    switch (receivedId) {
                        case 'menu_membros':
                            ...
                            await sendWhatsAppMessage(from, { type: 'text', text: { body: "Ok. Para começar, digite o seu *nome completo*:" } });
                            break;
                        case 'menu_eventos':
                            const eventosFromDB = await db.collection('events').find({}).sort({ title: 1 }).toArray();
                            if (eventosFromDB.length === 0) {
                                messagePayload = { type: 'text', text: { body: "No momento, não temos eventos com inscrições abertas." } };
                            } else {
                                const eventRows = eventosFromDB.map(e => ({ id: e._id.toString(), title: e.title, description: e.description }));
                                messagePayload = { type: "interactive", interactive: { type: "list", header: { type: "text", text: "Eventos Disponíveis" }, body: { text: "Temos os seguintes eventos abertos:" }, action: { button: "Ver Eventos", sections: [{ title: "Nossos Eventos", rows: eventRows }] } } };
                        ...
                        case 'menu_falar_equipe':
                            const userName = req.body.entry[0].changes[0].value.contacts[0].profile.name || from;
                            await db.collection('pendingConversations').updateOne({ _id: from }, { $set: { userNumber: from, userName, createdAt: new Date() } }, { upsert: true });
                            io.emit('new_pending_conversation', { userNumber: from, userName });
                            messagePayload = { type: 'text', text: { body: textos.falarComEquipe } };
                            if (ADMIN_WHATSAPP_NUMBER) await sendWhatsAppMessage(ADMIN_WHATSAPP_NUMBER, { type: 'text', text: { body: `🔔 *Alerta de Atendimento* 🔔\n\n*${userName}* (${from}) solicitou atendimento.` } });
                            break;
                        default: messagePayload = { type: 'text', text: { body: textos[receivedId] || "Opção inválida." } };
                    }
            ...
            if (message.interactive?.button_reply?.id !== 'menu_falar_equipe') {
                const timeoutId = setTimeout(async () => {
                    ...
                    await sendWhatsAppMessage(from, { type: 'text', text: { body: "Sua sessão expirou. Para começar de novo, envie qualquer mensagem." } });
...
io.on('connection', async (socket) => {
    const db = app.locals.db; // Ação Corretiva: Pega o 'db' do app.locals
