...
function startSessionTimeout(from, db) {
    // Cancela qualquer timeout anterior para este utilizador
    if (timeoutHandlers.has(from)) {
        clearTimeout(timeoutHandlers.get(from));
        timeoutHandlers.delete(from);
    }

    // Agenda um novo timeout
    const timeoutId = setTimeout(async () => {
        console.log(`[Timeout] Sessão expirada para ${from}.`);
        await sendWhatsAppMessage(from, { type: 'text', text: { body: textos.mensagemEncerramento } });
        await db.collection('userSessions').deleteOne({ _id: from });
        timeoutHandlers.delete(from);
    }, SESSAO_TIMEOUT_MINUTOS * 60 * 1000);

    // Armazena o novo handler de timeout
    ...
    switch (userState.step) {
        case 'select_event':
            ...
            if (!eventoSelecionado) {
                ...
                return; // Para a execução e aguarda uma nova seleção válida
            }
...
async function handleEventRegistration(from, message, db) {
    ...
    switch (userState.step) {
        case 'select_event':
            ...
            const eventoSelecionado = eventosDisponiveis.find(e => e.id === eventoId);
            if (!eventoSelecionado) {
                await sendWhatsAppMessage(from, { type: 'text', text: { body: "Opção de evento inválida. Por favor, selecione um da lista." } });
                return;
            }
            await db.collection('userSessions').updateOne({ _id: from }, { $set: { step: 'ask_name', evento: eventoSelecionado } });
            await sendWhatsAppMessage(from, { type: 'text', text: { body: `Ótima escolha! Para se inscrever no evento "${eventoSelecionado.title}", por favor, digite o seu nome completo:` } });
...
app.post('/upload-media', upload.single('mediaFile'), async (req, res) => {
    console.log('[Upload] Rota /upload-media alcançada.');
    const { to } = req.body;
    const file = req.file;

    if (!to || !file) {
        console.error('[Upload Falha] Faltam dados (destinatário ou ficheiro).');
        return res.status(400).json({ success: false, message: 'Faltam dados.' });
    }

    console.log(`[Upload] Preparando ficheiro: ${file.originalname} para ${to}`);
    const form = new FormData();
    form.append('messaging_product', 'whatsapp');
    form.append('file', fs.createReadStream(file.path), file.originalname);

    try {
        console.log('[Upload] Enviando ficheiro para a API da Meta...');
        ...
        console.log(`[Upload] Sucesso! Mídia carregada com ID: ${mediaId}`);

        const mediaType = file.mimetype.startsWith('image') ? 'image' : 'video';
        const messagePayload = { type: mediaType, [mediaType]: { id: mediaId } };
        
        await sendWhatsAppMessage(to, messagePayload);
        
        res.json({ success: true, message: 'Mídia enviada com sucesso!' });

    } catch (error) {
        const errorMessage = error.response?.data?.error?.message || error.message;
        console.error("[Upload] FALHA NO PROCESSO DE ENVIO DE MÍDIA:", errorMessage);
        res.status(500).json({ success: false, message: `Falha ao enviar: ${errorMessage}` });
    } finally {
        fs.unlink(file.path, (err) => {
            if (err) console.error("Erro ao apagar ficheiro temporário:", err);
        });
    }
});
...
app.post('/webhook', async (req, res) => {
    ...
    try {
        ...
        if (body.object === 'whatsapp_business_account' && body.entry?.[0]?.changes?.[0]?.value?.messages?.[0]) {
            ...
            if (timeoutHandlers.has(from)) {
                clearTimeout(timeoutHandlers.get(from));
                console.log(`[Timeout] Timeout cancelado para ${from} devido a nova atividade.`);
            }
            ...
            const receivedId = message.interactive?.list_reply?.id || message.interactive?.button_reply?.id;
            if (receivedId !== 'menu_falar_equipe') {
                startSessionTimeout(from, db);
                console.log(`[Timeout] Timeout de ${SESSAO_TIMEOUT_MINUTOS} min agendado para ${from}.`);
            }
        }
    } catch (error) {
